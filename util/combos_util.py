"""
    Module provides the utility functions for working with Commander Spellbook
"""
import time
import sys
import requests
...
def get():
    """Loads the all combo data into memory"""
...
def download():
    """Downloads all combo data to the drive"""
    if not os.path.isdir('data'):
        os.mkdir('data')
...
def pp(combos):
    """Prints all the combos in a slightly prettier format"""
    #print more data
...
def force_graph(combos):
    """Graphs the combos as nodes and links combos that share a card"""
    page = [
        '<head>\n',
...
def force_graph2(combos):
    """graphs the combos with cards and combos as nodes and combos linked to their included cards"""
    page = [
        '<head>\n',
        '  <style> body { margin: 0; } </style>\n',
        '  <script src="util/3d-force-graph/3d-force-graph.js"></script>\n',
